...
impl<'a> Coffee<'a> {
    ...
    fn get_import_from_symbol(&self, symbol: Symbol) -> Result<usize> {
        ...
        if polished_import_name.unwrap().contains("$") {
            ...
            let library_name_dll = format!("{}.dll", split_symbol_name.next().unwrap().to_lowercase());
            ...
            if function_name.is_some() {
                ...
                let load_library_address = unsafe {
                    LoadLibraryW(PCWSTR(
                        WideCString::from_str(format!("{}\0", library_name_dll))?.as_ptr() as *const _,
                    ))?
                };
            ...
            if section.pointer_to_raw_data != 0 {
                ...
                unsafe {
                    intrinsics::volatile_copy_nonoverlapping_memory(
                        ...
                        self.coff_buffer
                            .as_ptr()
                            .add(section.pointer_to_raw_data as usize) as *const u8,
                        section_size,
                    );
                }
            }
                        ...
                        Some((name, symbol)) => {
                            ...
                            if self.is_x64()? {
                                match relocation.typ {
                                    ...
                                    IMAGE_REL_AMD64_ADDR64 => {
                                        // The absolute address is the target section base + the relocation overwrite
                                        ...
                                        debug!(
                                            "Absolute address: {:#x}, relocation overwrite address: {:#x}",
                                    ...
                                    IMAGE_REL_AMD64_ADDR32NB => {
                                        let offset = unsafe {
                                            core::ptr::read_unaligned(relocation_overwrite_address as *const u32)
                                        };
                                ...
                                match relocation.typ {
                                    ...
                                    // The 32-bit relative displacement to the target. This supports the x86 relative
    ...
    fn execute_bof(
        ...
        // Check if BeaconDataParse, BeaconDataPtr, BeaconDataInt, BeaconDataShort, BeaconDataLength or
        // BeaconDataExtract is present on the mapped functions
        let data_functions = [
            "BeaconDataParse",
            "BeaconDataPtr",
            "BeaconDataInt",
        ...
        {
            warn!(
                "This BOF requires arguments but no arguments were passed! The BOF may crash or may not function \
                 properly!"
            );
        }
