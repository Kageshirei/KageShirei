...
async fn handle_command(db_pool: Pool, basic_response: BasicAgentResponse, raw_body: Bytes, headers: HeaderMap, cmd_request_id: String) -> Result<Bytes> {
		...
		Ok(protocol) => {
			match protocol {
				handlers::Protocol::Json => {
					...
					let response = handle_command(db_pool, data, body.clone(), headers, cmd_request_id).await?;
	...
	T: Deserialize,
{
	...
	tracing::Span::current().record("latency", humantime::format_duration(latency.round()).to_string());
