...
async fn get_agent_specs(cmd_request_id: &str, conn: &mut PooledConnection<AsyncDieselConnectionManager<AsyncPgConnection>>) -> Result<AgentSpecs> {
	let (agent_id, hostname) = agents_command_requests::table.inner_join(agents::table)
...
pub async fn handle_terminate(db_pool: Pool, cmd_request_id: String) -> Result<Bytes> {
	...
	tokio::join!(
		...
		CreateLog::new(srv_mod_database::schema_extension::LogLevel::WARN)
		...
		diesel::update(agents::table)
			...
			.set((
				agents::terminated_at.eq(Utc::now()),
			))
	);
