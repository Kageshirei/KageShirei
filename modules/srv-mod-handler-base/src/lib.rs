...
pub async fn handle_command_result(state: HandlerSharedState, mut body: Bytes, headers: HeaderMap, cmd_request_id: String) -> Response<Body> {
	...
	body = match state.config.security.encryption_scheme {
        ...
        EncryptionScheme::Symmetric => decrypt_symmetric_or_fail::decrypt_symmetric_or_fail(state.config.security.algorithm.as_ref(), body).unwrap(),
        EncryptionScheme::Asymmetric => decrypt_asymmetric_or_fail::decrypt_asymmetric_or_fail(state.config.security.algorithm.as_ref(), body).unwrap(),
	};
