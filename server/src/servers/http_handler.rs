...
pub fn spawn(config: Arc<HandlerConfig>, cancellation_token: CancellationToken, db: DatabaseConnection) -> tokio::task::JoinHandle<()> {
    let api_server_task = srv_mod_handler_http::start(
        config.clone(),
