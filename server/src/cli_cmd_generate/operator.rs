...
use srv_mod_entity::entities::user;
...
pub async fn generate_operator(
    ...
    let user = user::Entity::find().filter(user::Column::Username.eq(&args.username)).one(&db).await.map_err(|e| {
    ...
    let new_user = user::ActiveModel {
        ...
        password: Set(rs2_crypt::argon::Argon2::hash_password(args.password.as_str())?),
        ..Default::default()
    };
