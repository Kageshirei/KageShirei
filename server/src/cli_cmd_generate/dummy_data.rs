...
use srv_mod_database::schema_extension::AgentFields::NetworkInterfaces;
use srv_mod_database::schema_extension::{AgentFields, FilterOperator};
use srv_mod_database::{diesel, CUID2};

pub async fn make_dummy_data(config: SharedConfig) -> anyhow::Result<()> {
    ...
    let Some(mut connection) =
        srv_mod_database::migration::reset_all(readonly_config.database.url.as_str(), true)?
    else {
        unreachable!()
    };
