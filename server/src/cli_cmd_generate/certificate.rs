...
pub fn make_tls(args: &GenerateCertificateArguments) -> anyhow::Result<()> {
	...
	ca_params.distinguished_name.push(DnType::OrganizationName, args.organization_name.as_str());
	ca_params.distinguished_name.push(DnType::CommonName, args.common_name.as_str());
	...
	let not_before_pieces = args.not_before
	                            .split("-")
	...
	let not_after_pieces = args.not_after
	                           .split("-")
	                           .map(|x| x.parse::<u32>().unwrap())
	                           .collect::<Vec<u32>>();
	...
	std::fs::File::create(args.output_folder.join("cert.pem"))?.write_all(cert_pem.as_bytes())?;
	std::fs::File::create(args.output_folder.join("key.pem"))?.write_all(key_pem.as_bytes())?;
