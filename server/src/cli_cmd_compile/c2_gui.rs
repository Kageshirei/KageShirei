...
fn update_apt() -> anyhow::Result<()> {
	...
	if !status.success() {
		...
		return Err(anyhow::anyhow!("Failed to update apt"));
	}
...
fn upgrade_apt() -> anyhow::Result<()> {
	...
	if !status.success() {
		...
		return Err(anyhow::anyhow!("Failed to upgrade apt"));
	}
...
/// Update the apt package list
fn autoremove_apt() -> anyhow::Result<()> {
	...
	if !status.success() {
		...
		return Err(anyhow::anyhow!("Failed to run apt autoremove"));
	}
...
fn source_rust_environment() -> anyhow::Result<()> {
...
fn install_rust() -> anyhow::Result<()> {
	if Command::new("rustc").arg("--version").output().is_err() {
		...
		if !status.success() {
			...
			return Err(anyhow::anyhow!("Failed to install Rust"));
		}
		...
		if !status.success() {
			...
			return Err(anyhow::anyhow!("Failed to set the default Rust version"));
		}
		...
		if !status.success() {
			...
			return Err(anyhow::anyhow!("Failed to add the Windows target"));
		}
		...
		if !status.success() {
			...
			return Err(anyhow::anyhow!("Failed to add the Linux target"));
		}
	}
...
fn install_nvm() -> anyhow::Result<()> {
	if Command::new("nvm").arg("--version").output().is_err() {
		...
		if !status.success() {
			...
			return Err(anyhow::anyhow!("Failed to install NVM"));
		}
...
fn install_xwin() -> anyhow::Result<()> {
	...
	if !status.success() {
		...
		return Err(anyhow::anyhow!("Failed to install XWin"));
	}
...
/// Builds the client application
fn build_command_and_control() -> anyhow::Result<()> {
	...
	if !status.success() {
		...
		return Err(anyhow::anyhow!("Failed to build the client application"));
	}
