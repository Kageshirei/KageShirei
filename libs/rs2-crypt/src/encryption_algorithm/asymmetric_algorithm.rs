        ...
        T: SymmetricEncryptionAlgorithm + EncryptionAlgorithm + WithKeyDerivation,
{
    ...
    pub fn serialize_public_key(&self) -> Bytes { Bytes::from(self.public_key.to_sec1_bytes()) }
    ...
    pub fn derive_shared_secret(&mut self, public_key: Arc<PublicKey>) -> Hkdf<Sha3_512, SimpleHmac<Sha3_512>> {
        ...
        let shared_secret = k256::ecdh::diffie_hellman(&self.secret_key.to_nonzero_scalar(), public_key.as_affine());
...
mod test {
    ...
    fn test_asymmetric_algorithm() {
        ...
        let decrypted = alice
            .decrypt(encrypted, Some(used_key.unwrap().as_ref().clone()))
            .unwrap();
    ...
    fn test_key_automatically_rotate() {
        ...
        for _ in 0..=KEY_ROTATION_THRESHOLD + 1 {
            ...
            // shift the last encrypted and the previous round encrypted, after the check that the decryption is
            // successful we will ensure that they differ
            ...
            let decrypted = alice
                .decrypt(
                    encrypted,
                    Some(last_used_key.clone().unwrap().as_ref().clone()),
                )
                .unwrap();
            assert_eq!(data, decrypted);
